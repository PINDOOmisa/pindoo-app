...
import categoriesRaw from "@/data/categories"; // default export nebo uprav na { categories } dle tvého souboru
...
function normalize(raw: Raw): Cat | null {
  ...
  return {
    title,
    slug,
    description: normDesc(raw),
    iconUrl: normIcon(raw),
  };
}
...
export default function CategoryGrid() {
  // Podpora jak pro default export pole, tak pro { categories }
  const arr: Raw[] = Array.isArray(categoriesRaw)
    ? categoriesRaw
    : Array.isArray((categoriesRaw as any)?.categories)
    ? (categoriesRaw as any).categories
    : [];

  const normalized = arr.map(normalize).filter(Boolean) as Cat[];
  ...
  return (
    <section className="w-full">
      ...
      >
        ...
        {/* FEEDBACK PANEL (přidáváme vždy za grid) */}
        <div className="col-span-1 xs:col-span-2 md:col-span-3 lg:col-span-4 xl:col-span-5">
            ...
            <div className="flex flex-col md:flex-row items-center gap-4 p-4">
              ...
              <div className="text-center md:text-left">
                <p className="text-slate-800 font-medium">
                  Chybí tu nějaká kategorie nebo něco nesedí?
                </p>
                <p className="text-slate-600 text-sm">
                  Dej nám vědět a upravíme to. Díky za zpětnou vazbu!
                </p>
              </div>
      ...
      {/* Spodní „šedé podbarvení“ bloku jako celek */}
